    ...
    success: function(data) {
    	...
    	//alert(data);
		   ...
		   $(response).each(function(i,res) {
			...
			html+='<div class="col-sm-9 col-xs-12">';
			html+='<input type="text" name="email" value="'+res.email+'" class="form-control" id="inputEmail3" >';
			html+='</div>';
            html+='</div>';	 
				html+='<div class="form-group">';
			...
			html+='<option value="Germany" >Germany</option>';
			html+='<option value="Deutch">Deutch</option>';
       				 ...
       				 for(i in response){
							 ...
							 html += 'Ph : '+response[i].phoneNumber + '<br />';
          					 ...
          					 if(response[i].status == 'active' && response[i].arrivedVal == '0'){
          						 ...
          						 html += '<div class="flag_icon">';
          						 html += '<button class="btn btn-default btn-icon" type="button">';
          						 html += '<span class="glyphicon glyphicon-trash deleteHotelRecrdID " aria-hidden="true">';
          						 html += '<input type="hidden" id="deleteBtn" value="'+response[i].bookingId+'"/>';
          						 html += '</span>';
          						 html += '</button>';
          						 html += '</div>';
...
<script>
	...
	function primaryDates(date) {
		...
		$
				.ajax({
					url : "getPrimaryDatesList.htm?date=" + date,
					type : 'POST',
					success : function(data) {
					...
					success : function(data) {
						...
						for (var i = 0, key = Object.keys(response), ii = key.length; i < ii; i++) {
							if (i < 4) {
								...
								html += '<div class="cell cell_byobject cell_header sticker">'
										+ key[i] + '</div>';
	...
	function saveReservation(e) {
		...
		if (slctdSeats == 0) {
			var alertmsg = 'Please select atleast one seat to reserve';
				...
				.ajax({
					...
					success : function(data) {
						...
						for (key in response) {
							...
							html += '<div class="col-xs-11">';
							html += '<p>' + parsedKey.eventName + '</p>';
							html += '<select class="form-control" id="noOfGuestSeats">';
							html += '<option value="0">Select Tickets</option>';
							var seats = getIncrementalArray(parsedKey.availableSeats);
							for (var i = 1; i <= seats.length; i++) {
								if (i < 10) {
	...
	function saveGuestReservation(e) {
		...
		if (slctdSeats == 0) {
			var alertmsg = 'Please select atlest one seat to reserve';
			$('#guestSeatsStatus').html(alertmsg).fadeOut(4000);
			e.preventDefault();
		}
		//validations end -----------------------------------------------------------
...
<!-- ============================= CHANGE RESERVATION MODAL END =============================================-->			
						...
						<form class="form-horizontal">
							...
							<div class="button_padding">
								<div class="form-group">
									<div class="col-xs-12">
										<a href="javascript:void(0)" class="btn btn-default btn-save"
											onclick="saveReservation(); this.onclick=null;">Reserve
											table</a>
									</div>
									<!-- <div class="col-xs-6">
							...
							<form class="form-horizontal" id="reservationDiv">
								...
								<div class="col-xs-12" style="padding: 20px 20px 6px 20px;">
									<div class="form-group">
										...
										<div class="col-xs-11">
											<textarea class="form-control" id="guestNotes" rows="1"></textarea>
										</div>
									</div>
								...
								<div style="padding: 0px 100px;" class="button_padding">
									<div class="form-group">
										<div class="col-xs-12">
											<button type="button" class="btn btn-default btn-save"
												id="reserveSeats"
												onclick="saveGuestReservation(); this.onclick=null;">Reserve
												Now</button>
										</div>
										<!-- <div class="col-xs-4 nopadding" >
						  <a href="javascript:void(0)" class="btn btn-default btn-save" onclick="showReservationList()">Reservation List</a>
						</div> -->
										<!-- <div class="col-xs-6">
											<button type="button" class="btn btn-default btn-save"
												onclick="document.getElementById('myreservation_guest').style.display='none';document.getElementById('fade').style.display='none'">Cancel</button>
										</div> -->
									</div>
								</div>
							</form>
				...
				<div id="myeventreservation_details" class="white_content"
					...
					<a href="javascript:void(0)" class="close_popup"
						onclick="document.getElementById('myeventreservation_details').style.display='none';document.getElementById('fade').style.display='none'"><span
						class="glyphicon glyphicon-remove-circle" aria-hidden="true"></span></a>
			...
			<div class="col-xs-2 nopadding">
				<button type="button" class="btn btn-default btn-current-date"
